...
VS_OUTPUT SimVertexShader(VS_INPUT input)
{
    ...
    //output.pos.x = input.tex_idx.x / (float)nx;
    //output.pos.y = input.tex_idx.y / (float)ny;
    output.pos.z = 0.5f;
    output.pos.w = 1.0f;
...
float FrictionCalc(float h, float u)
{
    ...
    //return max(h*u*0.2f*0.2f, friction * h * abs(u) * u);
}
...
float4 Pass3( VS_OUTPUT input ) : SV_Target
{
    ...
    //const float3 result3 = in_state + d_by_dt * 0.2f;
...
float4 SouthBoundary( VS_OUTPUT input ) : SV_TARGET
{
    ...
    if (idx.x >= 200 && idx.x <= 250) {
        w_ghost = B + 40.0 + 10.0;
        hu_ghost = 0;
        hv_ghost = 40.0 * 0.01;//(-inflow_speed);
    }
...
GET_STATS_OUTPUT GetStats( VS_OUTPUT input )
{
    ...
    //output.target0 = float4(0.0, 1.0, 0.2, 1.0);
    //output.target1 = float4(1.0, 0.2, 0.0, 1.0);
    //output.target2 = 0.5;
